...
import { GET_CART_DETAILS } from '../../../../../components/CartPage/cartPage.gql';
...
export const useOfflinePayment = props => {
    ...
    const [cartDetailData] = useLazyQuery(GET_CART_DETAILS, {
    ...
    const [updatePMDetails, { error: pmMutationError, loading: paymentMethodMutationLoading }] = useMutation(
        setPaymentMethodDetailsOnCartMutation,
        {
            onCompleted: (data) => {
                if (data) {
    ...
    }, [isBillingAddressSame, billingAddressData]);
    ...
    useEffect(() => {
        try {
            if (shouldSubmit) {
                ...
                const hasErrors = !isBillingAddressSame && Object.keys(formState.errors).length;
