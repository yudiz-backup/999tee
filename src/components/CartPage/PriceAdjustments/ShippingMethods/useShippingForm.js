...
export const useShippingForm = props => {
    ...
    const handleZipChange = useCallback(
        zip => {
            ...
            if (zip !== selectedValues.zip) {
                if (zip && !zip.startsWith("0") && zip.length >= 6) {
                ...
                if (pincodeData && pincodeData.pincodecheck.status) {
        ...
        [apolloClient, localStorage.getItem('cart_id'), selectedValues.zip, shippingMethodsQuery]
    );
    ...
    const handleOnSubmit = useCallback(
        ...
        [localStorage.getItem('cart_id'), setShippingAddress]
    );
