...
const Price = props => {
    ...
    const { value, currencyCode, classes, price_range } = props;
    ...
    const parts = patches.toParts.call(
     ...
     const RangePrices = patches.toParts.call(
        new Intl.NumberFormat(locale, {
            style: 'currency',
            currency: currencyCode
        }),
...
const secondchildren = RangePrices.map((part, i) => {
    ...
    return <Fragment>{children} {price_range?.minimum_price?.final_price?.value !==  price_range?.maximum_price?.final_price?.value && <> - {secondchildren}</> }</Fragment>;
};
